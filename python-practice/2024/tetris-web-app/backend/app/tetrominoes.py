...
class Tetromino:
    ...
    def current_position(self) -> List[int]:
        ...
        """
        return self.get_positions(self._current_rotation)
    ...
    def rotate(self, grid: List[bool]) -> bool:
        ...
        temp_rotation_shape = self.get_positions(temp_rotation)
    ...
    def get_positions(self, rotation) -> List[int]:
        """Gets the slice of the 1D array intended to track the current rotation.
        Returns:
            List[int]: List of positions representing the current block on the grid.
        """
    ...
    def translate_left(self, grid: List[bool]) -> bool:
        ...
        """
        rotation = self.get_positions(self._current_rotation)
...
class JShape(Tetromino):
    def __init__(self):
        ref = (
            C.GRID_COLUMN_COUNT * C.GRID_ROW_COUNT - int(C.GRID_COLUMN_COUNT / 2) + 1
        )  # # Row 20 Col 6
        rotations = [
            ref - 1,
            ref,
